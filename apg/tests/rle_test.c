...
int main() {
  ...
  apg_rle_compress( (uint8_t*)input_buffer, input_sz, NULL, &compressed_sz ); // +1 to include the \0
  char* compressed_buffer = malloc( compressed_sz );
  apg_rle_compress( (uint8_t*)input_buffer, input_sz, (uint8_t*)compressed_buffer, &compressed_sz ); // +1 to include the \0
  printf( "input str: %s\noutput str: %s\n", input_buffer, compressed_buffer );
  float pc = ( (float)compressed_sz / (float)input_sz ) * 100.0f;
  printf( "input len=%u\noutput len=%u\n%.2f%% of original size\n", (uint32_t)input_sz, (uint32_t)compressed_sz, pc );

  printf( "-------------and decompression-----------\n" );
  {
    size_t decompressed_sz = 0;
    apg_rle_decompress( (uint8_t*)compressed_buffer, compressed_sz, NULL, &decompressed_sz );
    printf( "decompressed_sz=%u\n", (uint32_t)decompressed_sz );
    decompressed_buffer = malloc( decompressed_sz );
    apg_rle_decompress( (uint8_t*)compressed_buffer, compressed_sz, (uint8_t*)decompressed_buffer, &decompressed_sz );
