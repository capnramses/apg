...
void* apg_realloc (void* ptr, size_t sz, const char* file, int line) {
	...
	if (rptr) {
		...
		if (ptr) {
			...
			if (!found) {
				fprintf (stderr, "WARNING: apg_realloc did not find addr %p in list. "
					"from %s:%i\n", ptr, file, line);
			}
		// entirely new block
		} else {
			...
			g_apg_total_mallocd += sz;
			// leak checking data structure
			APG_Mem_Block* block_ptr = (APG_Mem_Block*)malloc (sizeof (
				APG_Mem_Block));
			block_ptr->addr = rptr;
			block_ptr->sz = sz;
			strncpy (block_ptr->file, file, 32);
...
void apg_free (void* ptr, const char* file, int line) {
	if (!ptr) {
		...
		if (!found) {
			fprintf (stderr, "WARNING: apg_free did not find addr %p in list. from "
				"%s:%i\n", ptr, file, line);
		}
		free (ptr);
	} // endif(!ptr)
...
void apg_mem_report () {
	...
	while (block_ptr) {
		fprintf (stderr, "LEAK: addr: %p sz %i. alloc at %s:%i\n", block_ptr->addr,
			(int)block_ptr->sz, block_ptr->file, block_ptr->line);
