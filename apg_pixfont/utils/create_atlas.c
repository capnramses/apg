...
int main() {
  ...
  for ( int i = 0; i < 256; i++ ) {
    char tmp[2] = { i, '\0' };
    int w = 0, h = 0;
    if ( !apg_pixfont_image_size_for_str( tmp, &w, &h, thickness, add_outline, 0 ) ) { continue; }
  ...
  for ( int i = 0; i < 256; i++ ) {
    ...
    if ( !apg_pixfont_str_into_image( tmp, subimg_ptr, cell_dims[0], cell_dims[1], n_chans, 0xFF, 0xFF, 0xFF, 0xFF, thickness, add_outline, 0 ) ) {
  ...
  int ret = stbi_write_png( "atlas.png", cell_dims[0] * 16, cell_dims[1] * 16, n_chans, atlas_ptr, cell_dims[0] * 16 * n_chans );
