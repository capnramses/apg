...
static uint32_t _atlas_index_for_sequence( const char* sequence, int* bytes_processed ) {
  ...
  *bytes_processed     = 0;
  ...
  if ( len > 1 ) {
    ...
    // Extras
    if ( codepoint == 0x0153 ) { atlas_index = 196; } // OE
    if ( codepoint == 0x0154 ) { atlas_index = 197; } // oe
    if ( codepoint == 0x01EA ) { atlas_index = 198; } // O tail
    if ( codepoint == 0x01EB ) { atlas_index = 199; } // o tail
  }
  return atlas_index;
}
