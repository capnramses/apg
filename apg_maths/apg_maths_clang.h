...
inline void print_vec3( vec3 v ) { printf( "[%.2f, %.2f, %.2f]\n", v.x, v.y, v.z ); }

inline void print_vec4( vec4 v ) { printf( "[%.2f, %.2f, %.2f, %.2f]\n", v.x, v.y, v.z, v.w ); }
...
inline void print_quat( versor q ) { printf( "[%.2f ,%.2f, %.2f, %.2f]\n", q[0], q[1], q[2], q[3] ); }
/*------------------------------VECTOR FUNCTIONS-----------------------------*/
inline float length_vec3( vec3 v ) { return sqrt( v.x * v.x + v.y * v.y + v.z * v.z ); }

// squared length
inline float length2_vec3( vec3 v ) { return v.x * v.x + v.y * v.y + v.z * v.z; }
...
inline float dot_vec3( vec3 a, vec3 b ) { return a.x * b.x + a.y * b.y + a.z * b.z; }
...
inline float vec3_to_heading( vec3 d ) { return atan2( -d.x, -d.z ) * ONE_RAD_IN_DEG; }
...
inline float dot_quat( versor q, versor r ) { return q[0] * r[0] + q[1] * r[1] + q[2] * r[2] + q[3] * r[3]; }
